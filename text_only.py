...
model.fit(X_train_txt, y_train, epochs=3, batch_size=64, validation_data=(X_val_txt, y_val))
_, val_accuracy = model.evaluate(X_val_txt, y_val)
print("验证集准确率：", val_accuracy)
end = time.time()
print("运行时间: %.3f秒" % (end - start))

'''predictions = model.predict(test_text_array)
